    ...
    @foreach ($getRecord as $value)
        <tr>
            ...
            <td style="min-width: 200px;">{{ $value->activity_title }}</td>
            <td style="min-width: 200px;">{{ $value->activity_type }}</td>
            <td style="min-width: 100px;">{{ date('d-m-Y', strtotime($value->date)) }}</td>
            {{-- <td style="min-width: 350px;">{{ $value->notes }}</td> --}}
            ...
            <td>
                @if (!empty($value->picture))
                    <a href="{{ asset('upload/farm_records/' . $value->picture) }}" target="_blank">
                        <img 
                            src="{{ asset('upload/farm_records/' . $value->picture) }}" 
                            alt="Picture" 
            ...
            <td style="min-width: 200px;">
                <a href="{{ route('farm_record.edit', [$value->id]) }}" class="btn btn-primary btn-sm">Edit</a>

                <form action="{{ url('admin/farm_record/delete/'.$value->id) }}" method="POST" class="d-inline-block delete-form">
                    @csrf
                    @method('DELETE')
