    ...
    @foreach ($getRecord as $value)
        <tr>
            ...
            <td style="min-width: 200px;">{{ $value->title }}</td>
            <td style="min-width: 100px;">{{ $value->category }}</td>
            <td style="min-width: 150px;">{{ $value->value }}</td>
            <td style="min-width: 100px;">{{ date('d-m-Y', strtotime($value->date)) }}</td>

            <td style="min-width: 350px;">
                @php
                    $fullText = $value->description;
            ...
            <td>
                @if (!empty($value->picture))
                    <a href="{{ asset('upload/miscellaneous/' . $value->picture) }}" target="_blank">
                        <img 
                            src="{{ asset('upload/miscellaneous/' . $value->picture) }}" 
                            alt="Picture" 
            ...
            <td style="min-width: 200px;">
                {{-- <a href="{{ route('miscellaneous.view', [$value->id]) }}" class="btn btn-warning btn-sm">View</a> --}}
                
                <a href="{{ route('miscellaneous.edit', [$value->id]) }}" class="btn btn-primary btn-sm">Edit</a>

                <form action="{{ url('admin/miscellaneous/delete/'.$value->id) }}" method="POST" class="d-inline-block delete-form">
                    @csrf
                    @method('DELETE')
