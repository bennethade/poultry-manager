    ...
    @foreach ($getRecord as $value)
        <tr>
            ...
            <td style="min-width: 100px;">
                <a href="{{ route('staff.sales.view', [$value->id]) }}" class="btn btn-warning btn-sm">View</a>
                
                {{-- <a href="{{ route('staff.sales.edit', [$value->id]) }}" class="btn btn-primary btn-sm">Edit</a>

                <form action="{{ url('staff/sales_record/daily_sales/delete/'.$value->id) }}" method="POST" class="d-inline-block delete-form">
                    @csrf
                    @method('DELETE')
