    ...
    @foreach ($getRecord as $value)
        <tr>
            ...
            <td style="min-width: 200px;">{{ $value->item_name }}</td>
            <td style="min-width: 200px;">{{ $value->category }}</td>
            <td style="min-width: 200px;">{{ $value->quantity }}</td>
            <td style="min-width: 100px;">{{ $value->cost }}</td>
            <td style="min-width: 150px;">{{ $value->source }}</td>


            <td>
                @if (!empty($value->picture))
                    <a href="{{ asset('upload/farm_inventory/' . $value->picture) }}" target="_blank">
                        <img 
                            src="{{ asset('upload/farm_inventory/' . $value->picture) }}" 
                            alt="Picture" 
            ...
            <td style="min-width: 150px;">
                <a href="{{ route('staff.farm_inventory.more_record', [$value->id]) }}" class="btn btn-secondary btn-sm">More Record</a>

                {{-- <a href="{{ route('staff.farm_inventory.edit', [$value->id]) }}" class="btn btn-primary btn-sm">Edit</a> --}}

                {{-- <form action="{{ url('staff/inventory/farm_inventory/delete/'.$value->id) }}" method="POST" class="d-inline-block delete-form">
                    @csrf
                    @method('DELETE')
