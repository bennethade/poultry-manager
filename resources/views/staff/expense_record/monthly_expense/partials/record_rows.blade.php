    ...
    @foreach ($getRecord as $value)
        <tr>
            ...
            <td style="min-width: 150px;">₦{{ number_format($value->opening_balance, 2) }}

            <td style="min-width: 150px;">₦{{ number_format($value->total_spent, 2) }}</td>
            <td style="min-width: 150px;">₦{{ number_format($value->closing_balance, 2) }}</td>
            ...
            {{-- <td style="min-width: 150px;">
                <a href="{{ route('staff.monthly_expenses.edit', [$value->id]) }}" class="btn btn-primary btn-sm">Edit</a>

                <form action="{{ url('staff/expense_record/monthly_expense_summary/delete/'.$value->id) }}" method="POST" class="d-inline-block delete-form">
                    @csrf
                    @method('DELETE')
