    ...
    @foreach ($getRecord as $value)
        <tr>
            ...
            {{-- <td style="min-width: 200px;">
                <a href="{{ route('staff.farm_daily_care.view', [$value->id]) }}" class="btn btn-warning btn-sm">View</a>
                
                <a href="{{ route('staff.farm_daily_care.edit', [$value->id]) }}" class="btn btn-primary btn-sm">Edit</a>

                <form action="{{ url('staff/general_farm_activity/farm_daily_care/delete/'.$value->id) }}" method="POST" class="d-inline-block delete-form">
                    @csrf
                    @method('DELETE')
